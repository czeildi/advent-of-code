...
diff_between_parts_stat %>%
  ...
  coord_flip() + 
  labs(x = NULL, y = 'time between solving part1 and part2', caption = "Green:  median\nRed: average") + 
  theme_bw()
...
part_completions %>%
  ...
  group_by(part_idx) %>% 
  mutate(rank_for_part = dplyr::min_rank(median_completion_seconds), part_idx = as.character(part_idx)) %>% 
